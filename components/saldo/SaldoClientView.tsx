...
interface SaldoClientViewProps {
...
export function SaldoClientView({
  initialBalance,
  initialHistory,
  initialUserName,
  cafeName,
}: SaldoClientViewProps) {
  const {
    balancePoints,
    maxCashIdr,
    formData,
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#bbcabf]/30 pb-5">
        ...
        <div className="flex items-center gap-2.5">
          <Link
            ...
            className="flex items-center gap-2 bg-[#eff4ff] text-[#006c49] border border-[#adedd3] px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-[#dce9ff] transition-all shadow-2xs"
          >
          ...
          <Link
            ...
            className="flex items-center gap-2 bg-[#006c49] text-white px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-[#005237] transition-all shadow-2xs"
          >
      ...
      {/* Status Real-time Pengajuan Aktif (Sedang Diproses) */}
      {activePayout ? (
        <PayoutStatusCard
          payout={activePayout}
